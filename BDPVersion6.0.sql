...
SELECT E.id, E.nombre, E.imagen_portada, C.nombre, A.autor, E.isbn, E.issn, E.doi, e.fecha_publicada, ED.editorial, F.formato, I.idioma, 
    STUFF((SELECT  ', ' + PC.palabra 
            FROM PALABRAS_CLAVE PC 
                INNER JOIN EJEMPLAR E 
                    ON PC.id_ejemplar = E.id
            FOR XML PATH ('')), 1,2, '') 'Palabras_clave'
FROM EJEMPLAR E
    ...
    INNER JOIN AUTOR A
        ON A.id_ejemplar = E.id
    INNER JOIN COLECCION C
        ON C.id = E.id_coleccion
GROUP BY E.id, E.nombre, E.imagen_portada, C.nombre, A.autor, E.isbn, E.issn, E.doi, e.fecha_publicada, ED.editorial, F.formato, I.idioma
ORDER BY E.id
