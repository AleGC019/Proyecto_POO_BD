...
/*
SELECT C.id, C.nombre, G.genero, T.tipo, A.nombre
FROM COLECCION C
INNER join genero G 
	on G.id = C.id_genero
inner join tipo T 
...
SELECT *
FROM EJEMPLAR, AUTOR
WHERE AUTOR.id_ejemplar = EJEMPLAR.id
...
INSERT INTO PALABRAS_CLAVE(palabra, id_ejemplar) VALUE("prueba", 2);
...
SELECT EJEMPLAR.id, EJEMPLAR.nombre, EJEMPLAR.imagen_portada, COLECCION.nombre AS 'cnombre', AUTOR.autor AS 'aautor', EJEMPLAR.isbn, issn, doi, fecha_publicada, EDITORIAL.editorial AS 'eeditorial', FORMATO.formato AS 'fformato', IDIOMA.idioma AS 'iidioma', 
    STRING_AGG(PALABRAS_CLAVE.palabra, ', ') 'Palabras_clave' 
    FROM EJEMPLAR 
        INNER JOIN EDITORIAL 
            ON EJEMPLAR.id_editorial = EDITORIAL.id 
        INNER JOIN IDIOMA 
            ON IDIOMA.id = EJEMPLAR.id_idioma 
        INNER JOIN FORMATO 
            ON FORMATO.id = EJEMPLAR.id_formato
        INNER JOIN AUTOR 
            ON AUTOR.id_ejemplar = EJEMPLAR.id 
        INNER JOIN COLECCION 
            ON COLECCION.id = EJEMPLAR.id_coleccion
        INNER JOIN PALABRAS_CLAVE
            ON PALABRAS_CLAVE.id_ejemplar = EJEMPLAR.id
...
SELECT EVENTO.id, EVENTO.titulo, EVENTO.imagen, EVENTO.asistentes_cantidad, EVENTO.incio_evento_hora_fecha, EVENTO.finalizacion_evento_hora_fecha, AREA.nombre, 
    STRING_AGG(OBJETIVOS.objetivos, ' ') 'Objetivos' 
    FROM EVENTO INNER JOIN AREA ON AREA.id = EVENTO.id_area
    INNER JOIN OBJETIVOS ON OBJETIVOS.id_evento = EVENTO.id
GROUP BY EVENTO.id, EVENTO.titulo, EVENTO.imagen, EVENTO.asistentes_cantidad, EVENTO.incio_evento_hora_fecha, EVENTO.finalizacion_evento_hora_fecha, AREA.nombre
